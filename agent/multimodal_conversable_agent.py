...
class MultimodalConversableAgent(ConversableAgent):
    ...
    def generate_oai_reply(
        ...
        #         "role": "user",
        #         "content": [
        #             {"type": "text", "text": "你好，你能干什么"},
        #         ],
        #     },
        # ]

        # # GPT->QWEN massage format transformation
        text_message = []
        for item in messages_with_b64_img:
            if item['role']=='system':
                text_message.append({"content": item['content'][0]['text'], "role": item['role']})
            else:
                context=[]
                context.append({"type": "text","text": item['content'][0]['text']})
                text_message.append({"content": context, "role": item['role']})

        # text_message[0]["content"] = "You are a helpful multimodal AI assistant.\nYou name now is Jerry!."
        # text_message[1]["content"][0]["text"] = "Hello, who are you?"
        response = client.create(context=messages[-1].pop("context", None), messages=text_message)
